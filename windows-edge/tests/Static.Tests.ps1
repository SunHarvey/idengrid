...
Assert-True ($install -match 'Release signing public key is not configured') 'Placeholder release keys must fail closed.'
...
Assert-True ($uninstall -match 'Assert-AllowedRoot' -and $uninstall -match '-Group \$FirewallGroup') 'Uninstall must use fixed roots and product-owned firewall rules.'
